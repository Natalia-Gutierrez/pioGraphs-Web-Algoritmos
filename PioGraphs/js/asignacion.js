...
function mostrarTablaMatriz(matriz, mCol, ubicacion, d, caso){
    ...
    if(caso==1){
        html+=`<th style="background-color : #f7d547;">Mínimos</th>`
    } else{
        html+=`<th style="background-color : #f7d547;">Máximos</th>`
    }
    html += `</tr>`;
    for (f = 0; f < d; f++) {
        html += `<tr><td>${getNombreOrigen(f)}</td>`;
        ...
        html+=`<td style="background-color : #f7d547;">${mCol[f]}</td>`
        html += `</tr>`;
    }
    html += `</table>`;
    $(ubicacion).html(html);
...
function mostrarTablaMatrizA1(matriz, mCol, ubicacion, d, caso){
    ...
    html+= `<tr>`;
    if (caso==1){
        html+= `<td style="background-color : #f7d547;"><strong>Mínimos</strong></td>`
    } else {
        html+= `<td style="background-color : #f7d547;"><strong>Máximos</strong></td>`
    }
    for (c=0; c<d; c++){
        html+=`<td style="background-color : #f7d547;">${mCol[c]}</td>`
    }
    html+=`</tr>`;
    html += `</table>`;
    $(ubicacion).html(html);
    ...
    for (f = 0; f < d; f++) {
        ...
        for (c = 0; c < d; c++) {
            if(coordenadas[f].fila==f&&coordenadas[f].columna==c){
                html += `<td style="background-color : #f7d547;">${matriz[f][c]}</td>`;
